...
function mt:onCastShot()
	...
	ac.wait(time,function()
		for i = 1,count do
			local p = point - {angle,distance/count * i - 300}
			local tbl = {2,3}
			...
			for j = 1,2 do
				...
				sg.animationSpeed(u,2.6 - 0.2 * i)
				sg.animationI(u,tbl[j])
				sg.set_color(u,{a = 0})
				unit[j] = u
			end
			ac.wait(0.5 + pulse * i,function()
				for i = 1,2 do
					unit[i]:remove()
				end
...
function mt:onCastStart()
	...
	sg.animationSpeed(hero,0.6/time)
	local mover = hero:moverLine
    {
		mover = hero,
		target = target,
